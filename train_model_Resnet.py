#train Resnet models (different versions)
...
def train_model11(eps, batchs, input_shape, lrs,lrs_nm,losses,losses_nm, x_train, y_train, x_test, y_test):
    
    for i in range(len(losses)):
        for j in range(len(lrs)):
            print('-----start training model11, adam '+str(lrs[j])+', '+losses_nm[i]+'-----')
            model=generator11(input_shape)
            model.compile(optimizer=Adam(learning_rate=lrs[j]), loss=losses[i], metrics=['accuracy'])
            csv_logger = CSVLogger(csvlogger_path+'model11'+'_adam'+lrs_nm[j]+'_'+losses_nm[i]+'_training.csv')
            filepath = checkpoint_path+'model11'+'_adam'+lrs_nm[j]+'_'+losses_nm[i]+'_{epoch:02d}_{loss:.4f}.hdf5'             
            ...
            print('-----finish training model11, adam '+str(lrs[j])+', '+losses_nm[i]+'-----')




def train_model12(eps, batchs, input_shape, lrs,lrs_nm,losses,losses_nm, x_train, y_train, x_test, y_test):
    
    for i in range(len(losses)):
        for j in range(len(lrs)):
            print('-----start training model12 , adam '+str(lrs[j])+', '+losses_nm[i]+'-----')
            model=generator12(input_shape)
            model.compile(optimizer=Adam(learning_rate=lrs[j]), loss=losses[i], metrics=['accuracy'])
            csv_logger = CSVLogger(csvlogger_path+'model12'+'_adam'+lrs_nm[j]+'_'+losses_nm[i]+'_training.csv')
            filepath = checkpoint_path+'model12'+'_adam'+lrs_nm[j]+'_'+losses_nm[i]+'_{epoch:02d}_{loss:.4f}.hdf5'             
            checkpoint = ModelCheckpoint(filepath, monitor = 'loss', verbose = 1, save_best_only = True, save_weights_only = True, mode = 'auto', save_freq = 'epoch', options = None)
            ...
            print('-----finish training model12, adam '+str(lrs[j])+', '+losses_nm[i]+'-----')
